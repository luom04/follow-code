...
if (!empty($_POST['dataContact'])) {
    ...
    /* Save data */
    if (($scoreCaptcha >= 0.5 && $actionCaptcha == 'validation_contact') || $testCaptcha == true) {
        $data = array();
...
$contact = $d->rawQueryOne("select content$lang from #_static where type = ? limit 0,1", array('lien-he'));
$hethongs = $d->rawQuery("select id, name$lang, desc$lang, iframe, photo from #_news where type = ? and find_in_set('hienthi',status) order by numb,id desc",array('chi-nhanh'));
