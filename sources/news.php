...
$banners = $d->rawQuery("select name$lang, link, photo from #_photo where type = ? and find_in_set('hienthi',status) order by numb, id desc", array('quang-cao'));
if ($id != '') {
    /* Lấy bài viết detail */
    $rowDetail = $d->rawQueryOne("select id, view, date_created, id_list, id_cat, id_item, id_sub, type, name$lang, slugvi, slugen, desc$lang, content$lang, iframe, photo,file_attach, options from #_news where id = ? and type = ? and find_in_set('hienthi',status) limit 0,1", array($id, $type));

    /* Lấy hình ảnh con */
    $rowDetailContent = $d->rawQuery("select * from #_gallery where id_parent = ? and com='news' and type = ? and kind='man' and val = ? and find_in_set('hienthi',status) order by numb,id desc", array($rowDetail['id'], $type, $type));
    /* Lấy tags */
    $newTags = $d->rawQuery("select id_tags from #_news_tags where id_parent = ?", array($rowDetail['id']));
    $newTags = (!empty($newTags)) ? $func->joinCols($newTags, 'id_tags') : array();
    if (!empty($newTags)) {
        $rowTags = $d->rawQuery("select name$lang, desc$lang, content$lang, slugvi, slugen, photo, attributes, id from #_tags where id in ($newTags) and find_in_set('hienthi',status) order by numb,id desc");
    }
    ...
    $sql = "select id, name$lang, slugvi, slugen, photo, date_created, desc$lang, file_attach, type from #_news where $where order by numb,id desc $limit";
    ...
    /* breadCrumbs */
    if (!empty($titleMain) && ($type != 'chinh-sach' && $type != 'huong-dan'))
        $breadcr->set($com, $titleMain);
    if (!empty($newsList))
        $breadcr->set($newsList[$sluglang], $newsList['name' . $lang]);
    if (!empty($newsCat))
        $breadcr->set($newsCat[$sluglang], $newsCat['name' . $lang]);
    if (!empty($newsItem))
        $breadcr->set($newsItem[$sluglang], $newsItem['name' . $lang]);
    if (!empty($newsSub))
        $breadcr->set($newsSub[$sluglang], $newsSub['name' . $lang]);
    $breadcr->set($rowDetail[$sluglang], $rowDetail['name' . $lang]);
    $breadcrumbs = $breadcr->get();

} else if ($idl != '') {
    /* Lấy cấp 1 detail */
    $newsList = $d->rawQueryOne("select id, name$lang, content$lang, slugvi, slugen, type, photo, options from #_news_list where id = ? and type = ? limit 0,1", array($idl, $type));
    ...
    if ($com == 'tin-tuc' || $com == 'tuyen-dung') {
        $where = " id_list = ? and type = ?  AND find_in_set('hienthi',status)";
        $params = array($idl, $type);
        /* LOAD BAN ĐẦU */
        $perPage = 8;
        $offset = 0;
        $sql = " select  id, name$lang, slugvi, slugen, photo,desc$lang, attributes,  date_created, type from #_news  where $where  ORDER BY numb, id DESC LIMIT $offset, $perPage  ";
        $news = $d->rawQuery($sql, $params);
        /* ĐẾM TỔNG */
        ...
        $sql = " SELECT id, name$lang, slugvi,slugen,photo,date_created, desc$lang,file_attach, attributes,type FROM #_news WHERE $where ORDER BY numb, id DESC LIMIT $start, $perPage ";
        $news = $d->rawQuery($sql, $params);
        $sqlNum = "SELECT COUNT(*) as num FROM #_news WHERE $where";
        $total = (int) $d->rawQueryOne($sqlNum, $params)['num'];
        $url = $func->getCurrentPageURL();
    ...
    // $curPage = $getPage;
    // $perPage = $type == 'tin-tuc' ? 32 : 12;
    // $startpoint = ($curPage * $perPage) - $perPage;
    // $limit = " limit " . $startpoint . "," . $perPage;
    // $sql = "select id, name$lang, slugvi, slugen, photo, date_created, desc$lang, file_attach from #_news where $where order by numb,id desc $limit";
    // $news = $d->rawQuery($sql, $params);
    // $sqlNum = "select count(*) as 'num' from #_news where $where order by numb,id desc";
    // $count = $d->rawQueryOne($sqlNum, $params);
    // $total = (!empty($count)) ? $count['num'] : 0;
    ...
    /* Lấy cấp 2 detail */
    $newsCat = $d->rawQueryOne("select id, id_list, name$lang, desc$lang, slugvi, slugen, type, photo, options from #_news_cat where id = ? and type = ? limit 0,1", array($idc, $type));

    $contentCate = $newsCat['desc' . $lang];
    /* Lấy cấp 1 */
    ...
    if (!empty($seoDB['title' . $seolang]))
        ...
        $seo->set('title', $newsCat['name' . $lang]);
    if (!empty($seoDB['keywords' . $seolang]))
        $seo->set('keywords', $seoDB['keywords' . $seolang]);
    if (!empty($seoDB['description' . $seolang]))
        $seo->set('description', $seoDB['description' . $seolang]);
    ...
    if (!empty($seoDB['title' . $seolang]))
        ...
        $seo->set('title', $newsItem['name' . $lang]);
    if (!empty($seoDB['keywords' . $seolang]))
        $seo->set('keywords', $seoDB['keywords' . $seolang]);
    if (!empty($seoDB['description' . $seolang]))
        $seo->set('description', $seoDB['description' . $seolang]);
    ...
    if (!empty($seoDB['title' . $seolang]))
        ...
        $seo->set('title', $newsSub['name' . $lang]);
    if (!empty($seoDB['keywords' . $seolang]))
        $seo->set('keywords', $seoDB['keywords' . $seolang]);
    if (!empty($seoDB['description' . $seolang]))
        $seo->set('description', $seoDB['description' . $seolang]);
    ...
    /* Lấy cấp 1 */
    if ($type == 'tin-tuc')
        $lists = $d->rawQuery("select id, name$lang, desc$lang, slugvi, slugen, photo from #_news_list where type = ? and find_in_set('hienthi',status) order by numb,id desc", array($type));
