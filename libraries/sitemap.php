...
echo '<url><loc>' . $configBase . '</loc><lastmod>' . date('c', time()) . '</lastmod><changefreq>daily</changefreq><priority>1.0</priority></url>';
if (!empty($requick)) {
    foreach ($requick as $value) {
        $func->createSitemap(
            ...
            "c",
            "daily",
            $value['priority_menu'] ?? '0.5',
            $value['priority_detail'] ?? '0.6',
            "vi",
            "numb,id",
            @$value['menu']
        );
