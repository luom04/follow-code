...
class Functions
{
    ...
    {
        // Lấy tham số cuối cùng nếu nó là string để xác định kiểu hiển thị
        $type = 'print'; // Mặc định
        $lastVar = end($vars);

        if (is_string($lastVar) && in_array($lastVar, ['print', 'json', 'var'])) {
            $type = array_pop($vars);
        }

        echo '<pre style="background: #f5f5f5; padding: 20px; border-radius: 5px; border: 1px solid #ddd; overflow-x: auto;">';
    ...
    function getTikTokVideo($videoUrl)
    {
        $apiUrl = "https://www.tiktok.com/oembed?url=" . $videoUrl;
    ...
    function getTikTokVideoId($url)
    {
        ...
        // Mẫu URL TikTok phổ biến
        if (preg_match('/tiktok\.com\/@[\w\.]+\/video\/(\d+)/', $url, $matches)) {
            return $matches[1];
        }

        // Mẫu URL rút gọn
        if (preg_match('/vm\.tiktok\.com\/(\w+)/', $url, $matches)) {
            // URL rút gọn cần được giải quyết thêm
            return "Cần giải quyết URL ngắn: " . $matches[1];
        }
    ...
    public function checkVideoLink($url)
    {
        // Loại bỏ khoảng trắng và chuẩn hóa URL
        $url = trim($url);
        ...
        if (
            preg_match('/(?:tiktok\.com\/)(?:@[^\/]+\/video\/|v\/|embed\/v2\/)(\d+)/', $url, $matches) ||
            preg_match('/(?:vm\.tiktok\.com\/)(\w+)/', $url, $matches)
        ) {
            $result['is_valid'] = true;
        ...
        // Không phải YouTube hoặc TikTok
        return $result;
    }
    ...
    public function writeJson($data = array())
    {
        ...
        if ($data['type'] == 'city') {
            ...
            if (!empty($data['idCity'])) {
                $ward = $this->d->rawQuery("select id, id_city, name from #_ward where id_city = ? and find_in_set('hienthi',status) order by numb,id desc", array($data['idCity']));
                /* Put data */
                $this->putJson('ward-' . $data['idCity'] . '.json', $ward);
            } else /* Create */ {
                $citys = $this->d->rawQuery("select id from #_city where find_in_set('hienthi',status)");
                if (!empty($citys)) {
                    $result = array();
                    foreach ($citys as $v_city) {
                        $ward = $this->d->rawQuery("select id, id_city, name from #_ward where id_city = ? and find_in_set('hienthi',status) order by numb,id desc", array($v_city['id']));
                        /* Put data */
                        if (!empty($ward)) {
                            $this->putJson('wards-' . $v_city['id'] . '.json', $ward);
                        }
                    }
                }
            }
    ...
    public function createSitemap(
        ...
        if ($menu == true && $field == 'id') {
            ...
            echo '<priority>' . $priorityMenu . '</priority>';
            echo '</url>';
        }
        ...
        if (!empty($sitemap)) {
            foreach ($sitemap as $value) {
                if (!empty($value['slug' . $lang])) {
                    ...
                    echo '<changefreq>' . $changefreq . '</changefreq>';
                    echo '<priority>' . $priorityDetail . '</priority>';
                    echo '</url>';
                }
            }
        }
...
*/
    ...
    public function checkRole()
    {
        ...
        if ((!empty($_SESSION[$loginAdmin]['role']) && $_SESSION[$loginAdmin]['role'] == 3))
            return false;
        else
            return true;
    }
    ...
    /* Check recaptcha v3 */
    public function checkRecaptcha2($response = '')
    {
        global $config;
        ...
        $active = !empty($config['googleAPI']['recaptcha']['active']);

        // Nếu tắt recaptcha thì cho qua
        if (!$active) {
            $result['test'] = true;
            $result['success'] = true;
            return $result;
        ...
        $url = $config['googleAPI']['recaptcha']['urlapi'] ?? 'https://www.google.com/recaptcha/api/siteverify';
        $secret = $config['googleAPI']['recaptcha']['secretkey'] ?? '';

        if (empty($secret)) {
            $result['error-codes'] = ['missing-input-secret'];
            return $result;
        }

        // Dùng POST cho ổn định, tránh bị block query string
        ...
        // Ưu tiên CURL (ổn định hơn file_get_contents)
        if (function_exists('curl_init')) {
            ...
            if ($raw === false || empty($raw)) {
                $result['error-codes'] = ['curl_failed', $err];
                return $result;
            }
        } else {
            ...
            if ($raw === false || empty($raw)) {
                $result['error-codes'] = ['file_get_contents_failed'];
                return $result;
            }
        }

        $recaptcha = json_decode($raw, true);
        if (!is_array($recaptcha)) {
            $result['error-codes'] = ['bad_json'];
            return $result;
        }

        $result['success'] = !empty($recaptcha['success']);
        $result['score'] = (float) ($recaptcha['score'] ?? 0);
        $result['action'] = (string) ($recaptcha['action'] ?? '');
        $result['hostname'] = (string) ($recaptcha['hostname'] ?? '');
    ...
    public function renderTable($title, $data)
    {
        ...
        foreach ($data as $key => $value) {
            echo "<tr>";
            echo "<td style='vertical-align:top; width:20%;'><strong>$key</strong></td>";
    ...
    public function uploadImage($file = '', $extension = '', $folder = '', $newname = '')
    {
        ...
        if (isset($_FILES[$file]) && !$_FILES[$file]['error']) {
            ...
            $MaxSize = $MaxSize[0];
            if ($_FILES[$file]['size'] > $MaxSize * 10485761) {
    ...
    public function getLists($ids = '')
    {
        ...
        if ($ids) {
            $temps = (!empty($ids)) ? explode(",", $ids) : array();
        }

        $row_lists = $this->d->rawQuery("select namevi, id from #_product_list where type = ? order by numb,id desc", array($type));

        $str = '<select id="id_lists" name="id_lists[]" class="select multiselect" multiple="multiple" >';
        for ($i = 0; $i < count($row_lists); $i++) {
            if (!empty($temps)) {
                if (in_array($row_lists[$i]['id'], $temps))
                    $selected = 'selected="selected"';
                else
                    $selected = '';
            } else {
            ...
            $str .= '<option value="' . $row_lists[$i]["id"] . '" ' . $selected . ' /> ' . $row_lists[$i]["namevi"] . '</option>';
        }
        $str .= '</select>';
    ...
    public function getAllCategory($table = '', $type = '', $id_danhmuc = '')
    {
        $sql = "SELECT 
                l.id   AS list_id,   l.namevi AS list_name,
                c.id   AS cat_id,    c.namevi AS cat_name,
                i.id   AS item_id,   i.namevi AS item_name,
                s.id   AS sub_id,    s.namevi AS sub_name
            FROM table_product_list AS l
            LEFT JOIN table_product_cat  AS c ON c.id_list  = l.id
            LEFT JOIN table_product_item AS i ON i.id_list  = l.id  AND i.id_cat  = c.id
            LEFT JOIN table_product_sub  AS s ON s.id_list  = l.id  AND s.id_cat  = c.id AND s.id_item = i.id
            WHERE l.type = :type
            ORDER BY l.numb ASC, l.id DESC, c.numb ASC, c.id DESC, i.numb ASC, i.id DESC, s.numb ASC, s.id DESC";

        $data = $this->d->rawQuery($sql, ['type' => $type]);
        $categories = [];
        foreach ($data as $row) {
            $lId = $row['list_id'];
            $cId = $row['cat_id'];
            ...
            // Cat
            if ($cId && !isset($categories[$lId]['cats'][$cId])) {
                $categories[$lId]['cats'][$cId] = [
                    'id' => $cId,
                    'name' => $row['cat_name'],
                    'items' => []
                ];
            ...
            // Item
            if ($iId && !isset($categories[$lId]['cats'][$cId]['items'][$iId])) {
                $categories[$lId]['cats'][$cId]['items'][$iId] = [
                    'id' => $iId,
                    'name' => $row['item_name'],
                    'subs' => []
                ];
            ...
            // Sub
            if ($sId) {
                $categories[$lId]['cats'][$cId]['items'][$iId]['subs'][$sId] = [
                    'id' => $sId,
                    'name' => $row['sub_name']
                ];
            }
        }
        $str = '<select id="id_danhmuc" name="id_danhmuc[]" class="select multiselect" multiple>';
        foreach ($categories as $list) {
            // list
            $selected = (strpos($id_danhmuc, 'list-' . $list['id']) !== false) ? 'selected' : '';
            $str .= '<option value="list-' . $list['id'] . '" ' . $selected . '>' . $list['name'] . '</option>';

            // cats
            if (!empty($list['cats'])) {
                foreach ($list['cats'] as $cat) {
                    $selected = (strpos($id_danhmuc, 'cat-' . $cat['id']) !== false) ? 'selected' : '';
                    $str .= '<option value="cat-' . $cat['id'] . '" data-parent="list-' . $list['id'] . '" ' . $selected . '>-- ' . $cat['name'] . '</option>';

                    // items
                    if (!empty($cat['items'])) {
                        foreach ($cat['items'] as $item) {
                            $selected = (strpos($id_danhmuc, 'item-' . $item['id']) !== false) ? 'selected' : '';
                            $str .= '<option value="item-' . $item['id'] . '" data-parent="cat-' . $cat['id'] . '" ' . $selected . '>---- ' . $item['name'] . '</option>';

                            // subs
                            if (!empty($item['subs'])) {
                                foreach ($item['subs'] as $sub) {
                                    $selected = (strpos($id_danhmuc, 'sub-' . $sub['id']) !== false) ? 'selected' : '';
                                    $str .= '<option value="sub-' . $sub['id'] . '" data-parent="item-' . $item['id'] . '" ' . $selected . '>------ ' . $sub['name'] . '</option>';
                                }
                            }
                        }
                    }
    ...
    /* Get product IDs */
    public function getProductIds($id = 0, $element = '', $table = '', $type = '')
    {
        $temps = [];

        /* Lấy danh sách id_muakem nếu có id truyền vào */
        if ($id) {
            $row = $this->d->rawQueryOne("select id_muakem from #_" . $table . " where id = ?", array($id));
            if (!empty($row['id_muakem'])) {
                $temps = explode(",", $row['id_muakem']);
            }
        }

        /* Lấy danh sách toàn bộ sản phẩm cùng type */
        $row_products = $this->cache->get(
            "select namevi, id from #_product where type = ? order by numb,id desc",
            array($type),
            "result",
            7200
        );
        ...
        foreach ($row_products as $p) {
            $selected = (in_array($p['id'], $temps)) ? 'selected="selected"' : '';
            $str .= '<option value="' . $p["id"] . '" ' . $selected . '>' . $p["namevi"] . ' (ID: ' . $p["id"] . ')</option>';
        }
        $str .= '</select>';
