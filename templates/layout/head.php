...
<!-- Favicon -->
<?php if (file_exists('upload/favicon/favicon.ico')) {
    $fv = '?v=' . filemtime('upload/favicon/favicon.ico'); ?>
    <link rel="icon" type="image/png" href="<?= ASSET ?>upload/favicon/favicon-96x96.png<?= $fv ?>" sizes="96x96" />
    <link rel="icon" type="image/svg+xml" href="<?= ASSET ?>upload/favicon/favicon.svg<?= $fv ?>" />
    <link rel="shortcut icon" href="<?= ASSET ?>upload/favicon/favicon.ico<?= $fv ?>" />
    <link rel="apple-touch-icon" sizes="180x180" href="<?= ASSET ?>upload/favicon/apple-touch-icon.png<?= $fv ?>" />
    <meta name="apple-mobile-web-app-title" content="<?= $setting['name' . $lang] ?>" />
    <link rel="manifest" href="<?= ASSET ?>upload/favicon/site.webmanifest<?= $fv ?>" />
<?php } else { ?>
    <link href="<?= ASSET . UPLOAD_PHOTO_L . ($favicon['photo'] ? $favicon['photo'] : $logo['photo']) ?>" rel="shortcut icon" type="image/x-icon" />
<?php } ?>
