...
if (!empty($_POST["listemail"]) && !empty($_POST['subject']) && !empty($_POST['content'])) {
    ...
    if ($emailer->send("customer", $arrayEmail, $subject, $message, $file)) {
        $func->transfer(emaildaduocguithanhcong, "newsletter?act=man&type=" . $type . "&p=" . $curPage);
    } else {
        $func->transfer(emailguibiloi, "newsletter?act=man&type=" . $type . "&p=" . $curPage, false);
    }
}
...
function viewMans()
{
    ...
    $url = "newsletter?act=man&type=" . $type;
...
function editMan()
{
    ...
    if (empty($id)) {
        $func->transfer(khongnhanduocdulieu, "newsletter?act=man&type=" . $type . "&p=" . $curPage, false);
    } else {
        $item = $d->rawQueryOne("select * from #_newsletter where id = ? and type = ? limit 0,1", array($id, $type));

        if (empty($item)) {
            $func->transfer(dulieukhongcothuc, "newsletter?act=man&type=" . $type . "&p=" . $curPage, false);
        }
    }
}
...
function saveMan()
{
    ...
    if (!empty($response)) {
        ...
        if (empty($id)) {
            $func->redirect("newsletter?act=add&type=" . $type);
        } else {
            $func->redirect("newsletter?act=edit&id=" . $id . "&type=" . $type);
        }
    }
    ...
    if ($id) {
        ...
        if ($d->update('newsletter', $data)) {
            ...
            $func->transfer(capnhatdulieuthanhcong, "newsletter?act=man&type=" . $type . "&p=" . $curPage);
        } else {
            $func->transfer(capnhatdulieubiloi, "newsletter?act=man&type=" . $type . "&p=" . $curPage, false);
        }
    } else {
        ...
        if ($d->insert('newsletter', $data)) {
            ...
            $func->transfer(luudulieuthanhcong, "newsletter?act=man&type=" . $type . "&p=" . $curPage);
        } else {
            $func->transfer(luudulieubiloi, "newsletter?act=man&type=" . $type . "&p=" . $curPage, false);
        }
    }
}
...
function deleteMan()
{
    ...
    if ($id) {
        ...
        if (!empty($row)) {
            ...
            $func->transfer(xoadulieuthanhcong, "newsletter?act=man&type=" . $type . "&p=" . $curPage);
        } else {
            $func->transfer(xoadulieubiloi, "newsletter?act=man&type=" . $type . "&p=" . $curPage, false);
        }
