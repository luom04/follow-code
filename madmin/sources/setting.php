...
function saveSetting()
{
    ...
    /* Check post */
    if (empty($_POST)) {
        $func->transfer(khongnhanduocdulieu, "setting?act=update", false);
    }
    ...
    if (!empty($response)) {
        ...
        $func->redirect("setting?act=update");
    }
    ...
    if (!empty($row)) {
        ...
        if ($d->update('setting', $data)) {
            ...
            $func->transfer(capnhatdulieuthanhcong, "setting?act=update");
        } else {
            $func->transfer(capnhatdulieubiloi, "setting?act=update", false);
        }
    } else {
        if ($d->insert('setting', $data)) {
            ...
            $func->transfer(luudulieuthanhcong, "setting?act=update");
        } else {
            $func->transfer(luudulieuthatbai, "setting?act=update", false);
        }
    }
}
