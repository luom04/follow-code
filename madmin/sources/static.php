...
function saveStatic()
{
    ...
    /* Check post */
    if (empty($_POST)) {
        $func->transfer(khongnhanduocdulieu, "static?act=update&type=" . $type, false);
    }
    ...
    if (!empty($config['static'][$type]['name'])) {
        ...
        if (!empty($response)) {
            ...
            $func->redirect("static?act=update&type=" . $type);
        }
    }
    ...
    if (!empty($static)) {
        ...
        if ($d->update('static', $data)) {
            ...
            if ($hash) {
                $d->rawQuery("update #_gallery set hash = ? where hash = ?", array('', $hash));
            }
            ...
            $func->transfer(capnhatdulieuthanhcong, "static?act=update&type=" . $type);
        } else {
            $func->transfer(capnhatdulieubiloi, "static?act=update&type=" . $type, false);
        }
    } else {
        if (!empty($data['namevi']) || !empty($data['descvi']) || !empty($data['contentvi'])) {
            ...
            if ($d->insert('static', $data)) {
                ...
                $func->transfer(luudulieuthanhcong, "static?act=update&type=" . $type);
            } else {
                $func->transfer(luudulieubiloi, "static?act=update&type=" . $type, false);
            }
        } else {
            $func->transfer(dulieurong, "static?act=update&type=" . $type, false);
        }
    }
}
