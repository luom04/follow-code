...
function editMan()
{
    global $d, $langadmin, $func, $curPage, $item, $attrs, $type;

    $id = (!empty($_GET['id'])) ? htmlspecialchars($_GET['id']) : 0;

    if (empty($id)) {
        $func->transfer(khongnhanduocdulieu, "tags?act=man&type=" . $type . "&p=" . $curPage, false);
    } else {
        $item = $d->rawQueryOne("select * from #_tags where id = ? and type = ? limit 0,1", array($id, $type));
        $attrs = json_decode($item['attributes'], true);

        if (empty($item)) {
            $func->transfer(dulieukhongcothuc, "tags?act=man&type=" . $type . "&p=" . $curPage, false);
        }
    }
}
...
function saveMan()
{
    ...
    if (!empty($response)) {
        ...
        if ($id) {
            $func->redirect("tags?act=edit&type=" . $type . "&p=" . $curPage . "&id=" . $id);
        } else {
            $func->redirect("tags?act=add&type=" . $type . "&p=" . $curPage);
        }
    }
    ...
    if ($id) {
        ...
        if ($d->update('tags', $data)) {
            ...
            $func->transfer(capnhatdulieuthanhcong, "tags?act=man&type=" . $type . "&p=" . $curPage);
        } else {
            $func->transfer(capnhatdulieubiloi, "tags?act=man&type=" . $type . "&p=" . $curPage, false);
        }
    } else {
        ...
        if ($d->insert('tags', $data)) {
            ...
            $func->transfer(luudulieuthanhcong, "tags?act=man&type=" . $type . "&p=" . $curPage);
        } else {
            $func->transfer(luudulieubiloi, "tags?act=man&type=" . $type . "&p=" . $curPage, false);
        }
    }
}
...
function deleteMan()
{
    ...
    if ($id) {
        ...
        if (!empty($row)) {
            ...
            $func->transfer(xoadulieuthanhcong, "tags?act=man&type=" . $type . "&p=" . $curPage);
        } else {
            $func->transfer(xoadulieubiloi, "tags?act=man&type=" . $type . "&p=" . $curPage, false);
        }
