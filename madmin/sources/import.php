...
function editImages()
{
    ...
    if (empty($id)) {
        $func->transfer(khongnhanduocdulieu, "import?act=man&type=" . $type . "&p=" . $curPage, false);
    } else {
        $item = $d->rawQueryOne("select * from #_excel where id = ? and type = ? limit 0,1", array($id, $type));

        if (empty($item)) {
            $func->transfer(dulieukhongcothuc, "import?act=man&type=" . $type . "&p=" . $curPage, false);
        }
    }
}
        ...
        if ($func->hasFile("file")) {
            ...
            if ($photo = $func->uploadImage("file", $config['import']['img_type'], UPLOAD_EXCEL, $file_name)) {
                ...
                if ($d->update('excel', $data)) {
                    $func->transfer(capnhatdulieuthanhcong, "import?act=man&type=" . $type . "&p=" . $curPage);
                } else {
                    $func->transfer(capnhatdulieubiloi, "import?act=man&type=" . $type . "&p=" . $curPage, false);
                }
            } else {
                $func->transfer(khongnhanduochinhanhmoi, "import?act=editImages&id=" . $id . "&type=" . $type . "&p=" . $curPage, false);
            }
        } else {
            $func->transfer(khongnhanduochinhanhmoi, "import?act=editImages&id=" . $id . "&type=" . $type . "&p=" . $curPage, false);
        }
...
function uploadImages()
{
    ...
    if (isset($_POST['uploadImg']) && isset($_FILES['files'])) {
        ...
        for ($i = 0; $i < $fileCount; $i++) {
            if (!in_array($myFile["name"][$i], $arr_file_delete, true)) {
                ...
                if ($d->insert('excel', $data)) {
                    ...
                    $func->transfer(luuhinhanhbiloi, "import?act=man&type=" . $type, false);
                }
        ...
        $func->transfer(luuhinhanhthanhcong, "import?act=man&type=" . $type);
    } else {
        $func->transfer(dulieurong, "import?act=man&type=" . $type, false);
    }
}
...
function deleteImages()
{
    ...
    if ($id) {
        ...
        if (!empty($row)) {
            ...
            $func->transfer(xoadulieuthanhcong, "import?act=man&type=" . $type . "&p=" . $curPage);
        } else {
            $func->transfer(xoadulieubiloi, "import?act=man&type=" . $type . "&p=" . $curPage, false);
        }
    ...
    if (isset($_POST['importExcel'])) {
        ...
        if ($file_type == "application/vnd.ms-excel" || $file_type == "application/x-ms-excel" || $file_type == "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet") {
            ...
            foreach ($objPHPExcel->getWorksheetIterator() as $worksheet) {
                ...
                for ($row = 2; $row <= $highestRow; ++$row) {
                    ...
                    if ($code != "") {
                        ...
                        $productImport = $d->rawQueryOne("select id, regular_price, sale_price, discount, photo from #_product where code = ? limit 0,1", array($code));
                        ...
                        $newRegular = (int)$regular_price;
                        $newSale    = (int)$sale_price;
                        $newPercent = (int)$discount;
                        ...
                        $data['regular_price'] = $newRegular;
                        $data['sale_price'] = $newSale;
                        $data['discount'] = $newPercent;
                        ...
                        if (isset($productImport['id']) && $productImport['id'] > 0) {
                            $oldRegular = (int)$productImport['regular_price'];
                            $oldSale    = (int)$productImport['sale_price'];
                            $oldPercent = (int)$productImport['discount'];

                            if ($newRegular > 0) {
                                // Nếu sale_price mới khác cũ → tính lại % discount
                                if ($newSale > 0 && $newSale !== $oldSale) {
                                    $data['discount'] = round((($newRegular - $newSale) / $newRegular) * 100);
                                }elseif ($newPercent > 0 && $newPercent !== $oldPercent) {
                                    if ($newRegular > 0 && $newPercent > 0) {
                                        $data['sale_price'] = ceil(($newRegular - ($newRegular * $newPercent / 100)) / 1000) * 1000;
                                    }
                                }
                            } else {
            ...
            if ($mess == '') {
                ...
                $func->transfer($mess, "import?act=man&type=" . $type);
            } else {
                $func->transfer($mess, "import?act=man&type=" . $type, false);
            }
        } else {
