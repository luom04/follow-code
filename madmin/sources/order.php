...
function editMan()
{
    ...
    if (empty($id)) {
        $func->transfer(khongnhanduocdulieu, "order?act=man&p=" . $curPage, false);
    } else {
        $item = $d->rawQueryOne("select * from #_order where id = ? limit 0,1", array($id));

        if (empty($item)) {
            $func->transfer(dulieukhongcothuc, "order?act=man&p=" . $curPage, false);
        } else {
...
function saveMan()
{
    ...
    if ($id) {
        ...
        if ($d->update('order', $data)) {
            $func->transfer(capnhatdulieuthanhcong, "order?act=man&p=" . $curPage);
        } else {
            $func->transfer(capnhatdulieubiloi, "order?act=man&p=" . $curPage, false);
        }
    } else {
        $func->transfer(dulieurong, "order?act=man&p=" . $curPage, false);
    }
}
...
function deleteMan()
{
    ...
    if ($id) {
        ...
        if (!empty($row)) {
            ...
            $func->transfer(xoadulieuthanhcong, "order?act=man&p=" . $curPage);
        } else {
            $func->transfer(xoadulieubiloi, "order?act=man&p=" . $curPage, false);
        }
