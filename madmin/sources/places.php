...
// $json = file_get_contents("tinh_thanh.json");
// $data = json_decode($json, true);
// $numC = $numD = 1;
// foreach ($data as $city) {
//     // Insert city
//     $sqlCity = "INSERT INTO table_city (id, name, slug, level, provinceid, code, numb, status, date_created, countryid)
//                 VALUES (?, ?, ?, ?, ?, ?, ?, 'hienthi', ?, 1)";
//     $d->rawQuery($sqlCity, [
//         $city['code'],
//         $city['name'],
...
//             $sqlWard = "INSERT INTO table_ward (id, id_city, wardcode, name, slug, code, level, numb, status, date_created)
//                         VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'hienthi', ?)";
//             $d->rawQuery($sqlWard, [
//                 $ward['code'],
//                 $city['code'],
...
function viewCitys()
{
    ...
    $url = "places?act=man_city" . $strUrl;
...
function editCity()
{
    ...
    if (empty($id)) {
        $func->transfer(khongnhanduocdulieu, "places?act=man_city&p=" . $curPage . $strUrl, false);
    } else {
        $item = $d->rawQueryOne("select * from #_city where id = ? limit 0,1", array($id));

        if (empty($item)) {
            $func->transfer(dulieukhongcothuc, "places?act=man_city&p=" . $curPage . $strUrl, false);
        }
    }
}
...
function saveCity()
{
    ...
    if (!empty($response)) {
        ...
        if ($id) {
            $func->redirect("places?act=edit_city&p=" . $curPage . $strUrl . "&id=" . $id);
        } else {
            $func->redirect("places?act=add_city&p=" . $curPage . $strUrl);
        }
    }
    ...
    if ($id) {
        ...
        if ($d->update('city', $data)) {
            ...
            $func->transfer(capnhatdulieuthanhcong, "places?act=man_city&p=" . $curPage . $strUrl);
        } else {
            $func->transfer(capnhatdulieubiloi, "places?act=man_city&p=" . $curPage . $strUrl, false);
        }
    } else {
        ...
        if ($d->insert('city', $data)) {
            $dataJson = array('type' => 'city');
            $func->writeJson($dataJson);
            $func->transfer(luudulieuthanhcong, "places?act=man_city&p=" . $curPage . $strUrl);
        } else {
            $func->transfer(luudulieubiloi, "places?act=man_city&p=" . $curPage . $strUrl, false);
        }
    }
}
...
function deleteCity()
{
    ...
    if ($id) {
        ...
        if (!empty($row)) {
            ...
            $dataJson = array('type' => 'city');
            $func->writeJson($dataJson);
            $func->transfer(xoadulieuthanhcong, "places?act=man_city&p=" . $curPage . $strUrl);
        } else {
            $func->transfer(xoadulieubiloi, "places?act=man_city&p=" . $curPage . $strUrl, false);
        }
...
function viewDistricts()
{
    ...
    $count = $d->rawQueryOne($sqlNum);
    $total = (!empty($count)) ? $count['num'] : 0;
    $url = "places?act=man_district" . $strUrl;
    $paging = $func->pagination($total, $perPage, $curPage, $url);
}
...
function editDistrict()
{
    ...
    if (empty($id)) {
        $func->transfer(khongnhanduocdulieu, "places?act=man_district&p=" . $curPage . $strUrl, false);
    } else {
        $item = $d->rawQueryOne("select * from #_district where id = ? limit 0,1", array($id));

        if (empty($item)) {
            $func->transfer(dulieukhongcothuc, "places?act=man_district&p=" . $curPage . $strUrl, false);
        }
    }
}
...
function saveDistrict()
{
    ...
    if (!empty($response)) {
        ...
        if ($id) {
            $func->redirect("places?act=edit_district&p=" . $curPage . $strUrl . "&id=" . $id);
        } else {
            $func->redirect("places?act=add_district&p=" . $curPage . $strUrl);
        }
    }
    ...
    if ($id) {
        ...
        if ($d->update('district', $data)) {
            ...
            $func->writeJson($dataJson);
            $func->transfer(capnhatdulieuthanhcong, "places?act=man_district&p=" . $curPage . $strUrl);
        } else {
            $func->transfer(capnhatdulieubiloi, "places?act=man_district&p=" . $curPage . $strUrl, false);
        }
    } else {
        $data['date_created'] = time();
        ...
        if ($d->insert('district', $data)) {
            ...
            $func->writeJson($dataJson);
            $func->transfer(luudulieuthanhcong, "places?act=man_district&p=" . $curPage . $strUrl);
        } else {
            $func->transfer(luudulieubiloi, "places?act=man_district&p=" . $curPage . $strUrl, false);
        }
    }
}
...
function deleteDistrict()
{
    ...
    if ($id) {
        ...
        if (!empty($row)) {
            ...
            $dataJson = array('type' => 'district');
            $func->writeJson($dataJson);
            $func->transfer(xoadulieuthanhcong, "places?act=man_district&p=" . $curPage . $strUrl);
        } else {
            $func->transfer(xoadulieubiloi, "places?act=man_district&p=" . $curPage . $strUrl, false);
        }
    } elseif (isset($_GET['listid'])) {
        $listid = explode(",", $_GET['listid']);
...
function viewWards()
{
    ...
    $count = $d->rawQueryOne($sqlNum);
    $total = (!empty($count)) ? $count['num'] : 0;
    $url = "places?act=man_ward" . $strUrl;
    $paging = $func->pagination($total, $perPage, $curPage, $url);
}
...
function editWard()
{
    ...
    if (empty($id)) {
        $func->transfer(khongnhanduocdulieu, "places?act=man_ward&p=" . $curPage . $strUrl, false);
    } else {
        $item = $d->rawQueryOne("select * from #_ward where id = ? limit 0,1", array($id));

        if (empty($item)) {
            $func->transfer(dulieukhongcothuc, "places?act=man_ward&p=" . $curPage . $strUrl, false);
        }
    }
}
...
function saveWard()
{
    ...
    if (!empty($response)) {
        ...
        if ($id) {
            $func->redirect("places?act=edit_ward&p=" . $curPage . $strUrl . "&id=" . $id);
        } else {
            $func->redirect("places?act=add_ward&p=" . $curPage . $strUrl);
        }
    }
    ...
    if ($id) {
        ...
        if ($d->update('ward', $data)) {
            $dataJson = array('type' => 'wards','idCity'=>$data['id_city']);
            $func->writeJson($dataJson);
            $func->transfer(capnhatdulieuthanhcong, "places?act=man_ward&p=" . $curPage . $strUrl);
        } else {
            $func->transfer(capnhatdulieubiloi, "places?act=man_ward&p=" . $curPage . $strUrl, false);
        }
    } else {
        $data['date_created'] = time();
        ...
        if ($d->insert('ward', $data)) {
            ...
            $func->writeJson($dataJson);
            $func->transfer(luudulieuthanhcong, "places?act=man_ward&p=" . $curPage . $strUrl);
        } else {
            $func->transfer(luudulieubiloi, "places?act=man_ward&p=" . $curPage . $strUrl, false);
        }
    }
}
...
function deleteWard()
{
    ...
    if ($id) {
        ...
        if (!empty($row)) {
            ...
            $dataJson = array('type' => 'wards');
            $func->writeJson($dataJson);
            $func->transfer(xoadulieuthanhcong, "places?act=man_ward&p=" . $curPage . $strUrl);
        } else {
            $func->transfer(xoadulieubiloi, "places?act=man_ward&p=" . $curPage . $strUrl, false);
        }
    } elseif (isset($_GET['listid'])) {
        $listid = explode(",", $_GET['listid']);
