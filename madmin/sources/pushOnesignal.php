...
function editMan()
{
    ...
    if (empty($id)) {
        $func->transfer(khongnhanduocdulieu, "pushOnesignal?act=man&p=" . $curPage, false);
    } else {
        $item = $d->rawQueryOne("select * from #_pushonesignal where id = ? limit 0,1", array($id));

        if (empty($item)) {
            $func->transfer(dulieukhongcothuc, "pushOnesignal?act=man&p=" . $curPage, false);
        }
    }
}
...
function saveMan()
{
    ...
    if (!empty($response)) {
        ...
        if ($id) {
            $func->redirect("pushOnesignal?act=edit&p=" . $curPage . "&id=" . $id);
        } else {
            $func->redirect("pushOnesignal?act=add&p=" . $curPage);
        }
    }
    ...
    if ($id) {
        ...
        if ($d->update('pushonesignal', $data)) {
            ...
            $func->transfer(capnhatdulieuthanhcong, "pushOnesignal?act=man&p=" . $curPage);
        } else {
            $func->transfer(capnhatdulieubiloi, "pushOnesignal?act=man&p=" . $curPage, false);
        }
    } else {
        if ($d->insert('pushonesignal', $data)) {
            ...
            $func->transfer(luudulieuthanhcong, "pushOnesignal?act=man&p=" . $curPage);
        } else {
            $func->transfer(luudulieubiloi, "pushOnesignal?act=man&p=" . $curPage, false);
        }
    }
}
...
function deleteMan()
{
    ...
    if ($id) {
        ...
        if (!empty($row)) {
            ...
            $func->transfer(xoadulieuthanhcong, "pushOnesignal?act=man&p=" . $curPage);
        } else {
            $func->transfer(xoadulieubiloi, "pushOnesignal?act=man&p=" . $curPage, false);
        }
...
function sendSync()
{
    ...
    if ($id) {
        ...
        $func->transfer(guithongbaothanhcong, "pushOnesignal?act=man&p=" . $curPage);
    } else {
        $func->transfer(khongnhanduocdulieu, "pushOnesignal?act=man&p=" . $curPage, false);
    }
}
