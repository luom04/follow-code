...
function saveSeoPage()
{
    ...
    /* Check post */
    if (empty($_POST)) {
        $func->transfer(khongnhanduocdulieu, "seopage?act=update&type=" . $type, false);
    }
    ...
    if (!empty($seopage)) {
        ...
        if ($d->update('seopage', $dataSeo)) {
            ...
            $func->transfer(capnhatdulieuthanhcong, "seopage?act=update&type=" . $type);
        } else {
            $func->transfer(capnhatdulieubiloi, "seopage?act=update&type=" . $type, false);
        }
    } else {
        if ($d->insert('seopage', $dataSeo)) {
            ...
            $func->transfer(luudulieuthanhcong, "seopage?act=update&type=" . $type);
        } else {
            $func->transfer(luudulieubiloi, "seopage?act=update&type=" . $type, false);
        }
    }
}
