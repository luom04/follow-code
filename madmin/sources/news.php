...
function viewMans()
{
    ...
    $url = "news?act=man" . $strUrl . "&type=" . $type;
...
function editMan()
{
    ...
    if (empty($id)) {
        $func->transfer(khongnhanduocdulieu, "news?act=man&type=" . $type . "&p=" . $curPage . $strUrl, false);
    } else {
        $item = $d->rawQueryOne("select * from #_news where id = ? and type = ? limit 0,1", array($id, $type));

        if (empty($item)) {
            $func->transfer(dulieukhongcothuc, "news?act=man&type=" . $type . "&p=" . $curPage . $strUrl, false);
        } else {
            /* Get gallery */
            if ($act != 'copy') {
...
function saveMan()
{
    ...
    if ($data) {
        foreach ($data as $column => $value) {
            if (strpos($column, 'content') !== false || strpos($column, 'desc') !== false || strpos($column, 'iframe') !== false) {
    ...
    if($data['id_list']){
        $list = $d->rawQueryOne("select namevi from #_news_list where id = ? and type = ? limit 0,1", array($data['id_list'], $type));
    ...
    if (!empty($response)) {
        ...
        if ($id || $act == 'save_copy') {
            if ($act == 'save_copy') {
                $func->redirect("news?act=copy&type=" . $type . "&p=" . $curPage . $strUrl . "&id_copy=" . $id);
            } else {
                $func->redirect("news?act=edit&type=" . $type . "&p=" . $curPage . $strUrl . "&id=" . $id);
            }
        } else {
            $func->redirect("news?act=add&type=" . $type . "&p=" . $curPage . $strUrl);
        }
    }
        ...
        if ($d->update('news', $data)) {
            ...
            if ($func->hasFile("file_attach")) {
                ...
                if ($file_attach = $func->uploadImage("file_attach", $config['news'][$type]['file_type'], UPLOAD_FILE, $file_name)) {
                    $row = $d->rawQueryOne("select id, file_attach from #_news where id = ? and type = ? limit 0,1", array($id, $type));
            ...
            if ($savehere) {
                $func->transfer(capnhatdulieuthanhcong, "news?act=edit&type=" . $type . "&p=" . $curPage . $strUrl . "&id=" . $id);
            } else {
                $func->transfer(capnhatdulieuthanhcong, "news?act=man&type=" . $type . "&p=" . $curPage . $strUrl);
            }
        } else {
            if ($savehere) {
                $func->transfer(capnhatdulieubiloi, "news?act=edit&type=" . $type . "&p=" . $curPage . $strUrl . "&id=" . $id, false);
            } else {
                $func->transfer(capnhatdulieubiloi, "news?act=man&type=" . $type . "&p=" . $curPage . $strUrl, false);
            }
        }
    } else {
        ...
        if ($d->insert('news', $data)) {
            ...
            if ($act == 'save_copy') {
                if($savehere || $buildSchema) {
                    $func->transfer(saochepdulieuthanhcong, "news?act=edit&type=" . $type . "&p=" . $curPage . $strUrl . "&id=" . $id_insert);
                } else {
                    $func->transfer(saochepdulieuthanhcong, "news?act=man&type=" . $type . "&p=" . $curPage . $strUrl);
                }
            } else {
                if($savehere || $buildSchema) {
                    $func->transfer(luudulieuthanhcong, "news?act=edit&type=" . $type . "&p=" . $curPage . $strUrl . "&id=" . $id_insert);
                } else {
                    $func->transfer(luudulieuthanhcong, "news?act=man&type=" . $type . "&p=" . $curPage . $strUrl);
                }
            }
        } else {
            if ($act == 'save_copy') {
                $func->transfer(saochepdulieubiloi, "news?act=copy&type=" . $type . "&p=" . $curPage . $strUrl . "&id=" . $id, false);
            } else {
                $func->transfer(luudulieubiloi, "news?act=copy&type=" . $type . "&p=" . $curPage . $strUrl . "&id=" . $id, false);
            }
        }
    }
}
...
function deleteMan()
{
    ...
    if ($id) {
        ...
        if (!empty($row)) {
            ...
            $func->transfer(xoadulieuthanhcong, "news?act=man&type=" . $type . "&p=" . $curPage . $strUrl);
        } else {
            $func->transfer(xoadulieubiloi, "news?act=man&type=" . $type . "&p=" . $curPage . $strUrl, false);
        }
...
function viewLists()
{
    ...
    $count = $d->rawQueryOne($sqlNum, array($type));
    $total = (!empty($count)) ? $count['num'] : 0;
    $url = "news?act=man_list&type=" . $type;
    $paging = $func->pagination($total, $perPage, $curPage, $url);
}
...
function editList()
{
    ...
    if (empty($id)) {
        $func->transfer(khongnhanduocdulieu, "news?act=man_list&type=" . $type . "&p=" . $curPage . $strUrl, false);
    } else {
        $item = $d->rawQueryOne("select * from #_news_list where id = ? and type = ? limit 0,1", array($id, $type));

        if (empty($item)) {
            $func->transfer(dulieukhongcothuc, "news?act=man_list&type=" . $type . "&p=" . $curPage . $strUrl, false);
        } else {
            /* Get gallery */
...
function saveList()
{
    ...
    if (!empty($response)) {
        ...
        if ($id) {
            $func->redirect("news?act=edit_list&type=" . $type . "&p=" . $curPage . $strUrl . "&id=" . $id);
        } else {
            $func->redirect("news?act=add_list&type=" . $type . "&p=" . $curPage . $strUrl);
        }
    }
    ...
    if ($id) {
        ...
        if ($d->update('news_list', $data)) {
            ...
            if ($func->hasFile("file1")) {
                ...
                if ($icon = $func->uploadImage("file1", $config['news'][$type]['img_type_list'], UPLOAD_NEWS, $file_name)) {
            ...
            if ($savehere) {
                $func->transfer(capnhatdulieuthanhcong, "news?act=edit_list&type=" . $type . "&p=" . $curPage . "&id=" . $id);
            } else {
                $func->transfer(capnhatdulieuthanhcong, "news?act=man_list&type=" . $type . "&p=" . $curPage . $strUrl);
            }

            $func->transfer(capnhatdulieuthanhcong, "news?act=man_list&type=" . $type . "&p=" . $curPage . $strUrl);
        } else {
            $func->transfer(capnhatdulieubiloi, "news?act=man_list&type=" . $type . "&p=" . $curPage . $strUrl, false);
        }
    } else {
        $data['date_created'] = time();
        ...
        if ($d->insert('news_list', $data)) {
            ...
            if ($savehere) {
                $func->transfer(luudulieuthanhcong, "news?act=edit_list&type=" . $type . "&p=" . $curPage . "&id=" . $id_insert);
            } else {
                $func->transfer(luudulieuthanhcong, "news?act=man_list&type=" . $type . "&p=" . $curPage . $strUrl);
            }

            $func->transfer(luudulieuthanhcong, "news?act=man_list&type=" . $type . "&p=" . $curPage . $strUrl);
        } else {
            $func->transfer(luudulieubiloi, "news?act=man_list&type=" . $type . "&p=" . $curPage . $strUrl, false);
        }
    }
}
...
function deleteList()
{
    ...
    if ($id) {
        ...
        if (!empty($row)) {
            ...
            $func->transfer(xoadulieuthanhcong, "news?act=man_list&type=" . $type . "&p=" . $curPage . $strUrl);
        } else {
            $func->transfer(xoadulieubiloi, "news?act=man_list&type=" . $type . "&p=" . $curPage . $strUrl, false);
        }
    } elseif (isset($_GET['listid'])) {
        $listid = explode(",", $_GET['listid']);
...
function viewCats()
{
    ...
    $count = $d->rawQueryOne($sqlNum, array($type));
    $total = (!empty($count)) ? $count['num'] : 0;
    $url = "news?act=man_cat" . $strUrl . "&type=" . $type;
    $paging = $func->pagination($total, $perPage, $curPage, $url);
}
...
function editCat()
{
    ...
    if (empty($id)) {
        $func->transfer(khongnhanduocdulieu, "news?act=man_cat&type=" . $type . "&p=" . $curPage . $strUrl, false);
    } else {
        $item = $d->rawQueryOne("select * from #_news_cat where id = ? and type = ? limit 0,1", array($id, $type));

        if (empty($item)) {
            $func->transfer(dulieukhongcothuc, "news?act=man_cat&type=" . $type . "&p=" . $curPage . $strUrl, false);
        }
    }
}
...
function saveCat()
{
    ...
    if (!empty($response)) {
        ...
        if ($id) {
            $func->redirect("news?act=edit_cat&type=" . $type . "&p=" . $curPage . $strUrl . "&id=" . $id);
        } else {
            $func->redirect("news?act=add_cat&type=" . $type . "&p=" . $curPage . $strUrl);
        }
    }
    ...
    if ($id) {
        ...
        if ($d->update('news_cat', $data)) {
            ...
            if ($savehere) {
                $func->transfer(capnhatdulieuthanhcong, "news?act=edit_cat&type=" . $type . "&p=" . $curPage . "&id=" . $id);
            } else {
                $func->transfer(capnhatdulieuthanhcong, "news?act=man_cat&type=" . $type . "&p=" . $curPage . $strUrl);
            }
            
        } else {
            $func->transfer(capnhatdulieubiloi, "news?act=man_cat&type=" . $type . "&p=" . $curPage . $strUrl, false);
        }
    } else {
        $data['date_created'] = time();
        ...
        if ($d->insert('news_cat', $data)) {
            ...
            if ($savehere) {
                $func->transfer(luudulieuthanhcong, "news?act=edit_cat&type=" . $type . "&p=" . $curPage . "&id=" . $id_insert);
            } else {
                $func->transfer(luudulieuthanhcong, "news?act=man_cat&type=" . $type . "&p=" . $curPage . $strUrl);
            }

        } else {
            $func->transfer(luudulieubiloi, "news?act=man_cat&type=" . $type . "&p=" . $curPage . $strUrl, false);
        }
    }
}
...
function deleteCat()
{
    ...
    if ($id) {
        ...
        if (!empty($row)) {
            ...
            $func->transfer(xoadulieuthanhcong, "news?act=man_cat&type=" . $type . "&p=" . $curPage . $strUrl);
        } else {
            $func->transfer(xoadulieubiloi, "news?act=man_cat&type=" . $type . "&p=" . $curPage . $strUrl, false);
        }
    } elseif (isset($_GET['listid'])) {
        $listid = explode(",", $_GET['listid']);
...
function viewItems()
{
    ...
    $count = $d->rawQueryOne($sqlNum, array($type));
    $total = (!empty($count)) ? $count['num'] : 0;
    $url = "news?act=man_item" . $strUrl . "&type=" . $type;
    $paging = $func->pagination($total, $perPage, $curPage, $url);
}
...
function editItem()
{
    ...
    if (empty($id)) {
        $func->transfer(khongnhanduocdulieu, "news?act=man_item&type=" . $type . "&p=" . $curPage . $strUrl, false);
    } else {
        $item = $d->rawQueryOne("select * from #_news_item where id = ? and type = ? limit 0,1", array($id, $type));

        if (empty($item)) {
            $func->transfer(dulieukhongcothuc, "news?act=man_item&type=" . $type . "&p=" . $curPage . $strUrl, false);
        }
    }
}
...
function saveItem()
{
    ...
    if (!empty($response)) {
        ...
        if ($id) {
            $func->redirect("news?act=edit_item&type=" . $type . "&p=" . $curPage . $strUrl . "&id=" . $id);
        } else {
            $func->redirect("news?act=add_item&type=" . $type . "&p=" . $curPage . $strUrl);
        }
    }
    ...
    if ($id) {
        ...
        if ($d->update('news_item', $data)) {
            ...
            if ($savehere) {
                $func->transfer(capnhatdulieuthanhcong, "news?act=edit_item&type=" . $type . "&p=" . $curPage . "&id=" . $id);
            } else {
                $func->transfer(capnhatdulieuthanhcong, "news?act=man_item&type=" . $type . "&p=" . $curPage . $strUrl);
            }
            
        } else $func->transfer(capnhatdulieubiloi, "news?act=man_item&type=" . $type . "&p=" . $curPage . $strUrl, false);
    } else {
        $data['date_created'] = time();
        ...
        if ($d->insert('news_item', $data)) {
            ...
            if ($savehere) {
                $func->transfer(luudulieuthanhcong, "news?act=edit_item&type=" . $type . "&p=" . $curPage . "&id=" . $id_insert);
            } else {
                $func->transfer(luudulieuthanhcong, "news?act=man_item&type=" . $type . "&p=" . $curPage . $strUrl);
            }
            
        } else $func->transfer(luudulieubiloi, "news?act=man_item&type=" . $type . "&p=" . $curPage . $strUrl, false);
    }
}
...
function deleteItem()
{
    ...
    if ($id) {
        ...
        if (!empty($row)) {
            ...
            $func->transfer(xoadulieuthanhcong, "news?act=man_item&type=" . $type . "&p=" . $curPage . $strUrl);
        } else $func->transfer(xoadulieubiloi, "news?act=man_item&type=" . $type . "&p=" . $curPage . $strUrl, false);
    } elseif (isset($_GET['listid'])) {
        $listid = explode(",", $_GET['listid']);
        ...
        $func->transfer(xoadulieuthanhcong, "news?act=man_item&type=" . $type . "&p=" . $curPage . $strUrl);
    } else $func->transfer(khongnhanduocdulieu, "news?act=man_item&type=" . $type . "&p=" . $curPage . $strUrl, false);
}
...
function viewSubs()
{
    ...
    $count = $d->rawQueryOne($sqlNum, array($type));
    $total = (!empty($count)) ? $count['num'] : 0;
    $url = "news?act=man_sub" . $strUrl . "&type=" . $type;
    $paging = $func->pagination($total, $perPage, $curPage, $url);
}
...
function editSub()
{
    ...
    if (empty($id)) {
        $func->transfer(khongnhanduocdulieu, "news?act=man_sub&type=" . $type . "&p=" . $curPage . $strUrl, false);
    } else {
        $item = $d->rawQueryOne("select * from #_news_sub where id = ? and type = ? limit 0,1", array($id, $type));

        if (empty($item)) {
            $func->transfer(dulieukhongcothuc, "news?act=man_sub&type=" . $type . "&p=" . $curPage . $strUrl, false);
        }
    }
}
...
function saveSub()
{
    ...
    if (!empty($response)) {
        ...
        if ($id) {
            $func->redirect("news?act=edit_sub&type=" . $type . "&p=" . $curPage . $strUrl . "&id=" . $id);
        } else {
            $func->redirect("news?act=add_sub&type=" . $type . "&p=" . $curPage . $strUrl);
        }
    }
    ...
    if ($id) {
        ...
        if ($d->update('news_sub', $data)) {
            ...
            if ($savehere) {
                $func->transfer(capnhatdulieuthanhcong, "news?act=edit_sub&type=" . $type . "&p=" . $curPage . "&id=" . $id);
            } else {
                $func->transfer(capnhatdulieuthanhcong, "news?act=man_sub&type=" . $type . "&p=" . $curPage . $strUrl);
            }
            
        } else {
            $func->transfer(capnhatdulieubiloi, "news?act=man_sub&type=" . $type . "&p=" . $curPage . $strUrl, false);
        }
    } else {
        $data['date_created'] = time();
        ...
        if ($d->insert('news_sub', $data)) {
            ...
            if ($savehere) {
                $func->transfer(luudulieuthanhcong, "news?act=edit_sub&type=" . $type . "&p=" . $curPage . "&id=" . $id_insert);
            } else {
                $func->transfer(luudulieuthanhcong, "news?act=man_sub&type=" . $type . "&p=" . $curPage . $strUrl);
            }
            
        } else {
            $func->transfer(luudulieubiloi, "news?act=man_sub&type=" . $type . "&p=" . $curPage . $strUrl, false);
        }
    }
}
...
function deleteSub()
{
    ...
    if ($id) {
        ...
        if (!empty($row)) {
            ...
            $func->transfer(xoadulieuthanhcong, "news?act=man_sub&type=" . $type . "&p=" . $curPage . $strUrl);
        } else $func->transfer(xoadulieubiloi, "news?act=man_sub&type=" . $type . "&p=" . $curPage . $strUrl, false);
    } elseif (isset($_GET['listid'])) {
        $listid = explode(",", $_GET['listid']);
