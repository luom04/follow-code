...
function editMan()
{
    ...
    if (empty($id)) {
        $func->transfer(khongnhanduocdulieu, "contact?act=man&p=" . $curPage, false);
    } else {
        $item = $d->rawQueryOne("select * from #_contact where id = ? limit 0,1", array($id));

        if (empty($item)) {
            $func->transfer(dulieukhongcothuc, "contact?act=man&p=" . $curPage, false);
        }
    }
}
...
function saveMan()
{
    ...
    if (!empty($response)) {
        ...
        $func->redirect("contact?act=edit&id=" . $id);
    }
    ...
    if ($id) {
        ...
        $d->where('id', $id);
        if ($d->update('contact', $data)) $func->transfer(capnhatdulieuthanhcong, "contact?act=man&p=" . $curPage);
        else $func->transfer(capnhatdulieubiloi, "contact?act=man&p=" . $curPage, false);
    } else {
        $func->transfer(dulieurong, "contact?act=man&p=" . $curPage, false);
    }
}
...
function deleteMan()
{
    ...
    if ($id) {
        ...
        if (!empty($row)) {
            ...
            $func->transfer(xoadulieuthanhcong, "contact?act=man&p=" . $curPage);
        } else {
            $func->transfer(xoadulieubiloi, "contact?act=man&p=" . $curPage, false);
        }
