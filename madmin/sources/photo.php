...
function savePhotoStatic()
{
    ...
    if (!empty($response)) {
        ...
        $func->redirect("photo?act=photo_static&type=" . $type);
    }
    ...
    if ($id) {
        ...
        if ($d->update('photo', $data)) {
            ...
            if ($func->hasFile("file")) {
                ...
                if ($photo = $func->uploadImage("file", $config['photo']['photo_static'][$type]['img_type'], UPLOAD_PHOTO, $file_name)) {
                    ...
                    unset($photoUpdate);/* Favicon: sinh bộ icon đa nền tảng vào upload/favicon/ */
                    if ($type == 'favicon') generateFaviconSet($photo);
                }
            }

            /* File video */
            if ($func->hasFile("file_video")) {
                $fileUpdate = array();
                $file_name = $func->uploadName($_FILES["file_video"]["name"]);

                if ($file_video = $func->uploadImage("file_video", $config['photo']['photo_static'][$type]['file_type_video'], UPLOAD_FILE, $file_name)) {
                    $row = $d->rawQueryOne("select id, file_video from #_photo where id = ? and act = ? and type = ? limit 0,1", array($id, 'photo_static', $type));

                    if (!empty($row)) {
                        $func->deleteFile(UPLOAD_FILE . $row['file_video']);
                    }

                    $fileUpdate['file_video'] = $file_video;
                    $d->where('id', $id);
            ...
            $func->transfer(capnhatdulieuthanhcong, "photo?act=photo_static&type=" . $type);
        } else {
            $func->transfer(capnhatdulieubiloi, "photo?act=photo_static&type=" . $type, false);
        }
    } else {
        ...
        if ($d->insert('photo', $data)) {
            ...
            $func->transfer(luudulieuthanhcong, "photo?act=photo_static&type=" . $type);
        } else {
            $func->transfer(luudulieubiloi, "photo?act=photo_static&type=" . $type, false);
        }
    }
}

function generateFaviconSet($photo)
{
    global $func, $setting, $type;

    $appTitle = (!empty($setting['namevi'])) ? $func->decodeHtmlChars($setting['namevi']) : $_SERVER['HTTP_HOST'];

    $generator = new FaviconGenerator();
    if (!$generator->generate(UPLOAD_PHOTO . $photo, $appTitle, '../upload/favicon/', '../favicon.ico')) {
        $func->transfer('Đã lưu ảnh nhưng tạo bộ favicon bị lỗi: ' . $generator->error, "photo?act=photo_static&type=" . $type, false);
    }
}
/* View photo */
function viewPhotos()
{
    ...
    $url = "photo?act=man_photo&type=" . $type;
...
function editPhoto()
{
    ...
    if (empty($id)) {
        $func->transfer(khongnhanduocdulieu, "photo?act=man_photo&type=" . $type . "&p=" . $curPage, false);
    } else {
        $item = $d->rawQueryOne("select * from #_photo where id = ? and act <> ? and type = ? limit 0,1", array($id, 'photo_static', $type));

        if (empty($item)) {
            $func->transfer(dulieukhongcothuc, "photo?act=man_photo&type=" . $type . "&p=" . $curPage, false);
        }
    }
}
...
function savePhoto()
{
    ...
    if ($id) {
        ...
        if (!empty($response)) {
            ...
            $message = base64_encode(json_encode($response));
            $flash->set('message', $message);
            $func->redirect("photo?act=edit_photo&type=" . $type . "&id=" . $id);
        }
        ...
        if ($d->update('photo', $data)) {
            ...
            if ($func->hasFile("video")) {
                ...
                if ($video = $func->uploadImage("video", $config['photo']['man_photo'][$type]['video_type'], UPLOAD_PHOTO, $file_name)) {
                    $row = $d->rawQueryOne("select id, link_video from #_photo where id = ? and type = ? limit 0,1", array($id, $type));
                    if (!empty($row)) {
                        $func->deleteFile(UPLOAD_PHOTO . $row['link_video']);
                    }
                    $photoUpdate['link_video'] = $video;
                    $d->where('type', $type);
                    $d->where('id', $id);
                    $d->update('photo', $photoUpdate);
                    unset($photoUpdate);
            ...
            if ($savehere) {
                $func->transfer(capnhatdulieuthanhcong, "photo?act=edit_photo&type=" . $type . "&p=" . $curPage . "&id=" . $id);
            } else {
                $func->transfer(capnhatdulieuthanhcong, "photo?act=man_photo&type=" . $type . "&p=" . $curPage);
            }
        } else {
            $func->transfer(capnhatdulieubiloi, "photo?act=man_photo&type=" . $type . "&p=" . $curPage, false);
        }
    } else {
        ...
        if ($numberPhoto && $dataMultiTemp) {
            ...
            for ($i = 0; $i < count($dataMultiTemp); $i++) {
                ...
                if (isset($config['photo']['man_photo'][$type]['name_photo']) && $config['photo']['man_photo'][$type]['name_photo'] == true) {
                    $success_name = true;
                }
                ...
                // if (($success_name || $success_photo || $success_video || $success_redirect) && empty($response)) {
                if (($success_photo || $success_video || $success_redirect || !empty($dataMulti['name'])) && empty($response)) {
                    ...
                    if ($d->insert('photo', $dataMulti)) {
                        ...
                        if (isset($config['photo']['man_photo'][$type]['video_photo']) && $config['photo']['man_photo'][$type]['video_photo'] == true) {
                            if ($func->hasFile("video" . $i)) {
                                $photoUpdate = array();
                                $file_name = $func->uploadName($_FILES['video' . $i]["name"]);

                                if ($video = $func->uploadImage("video" . $i, $config['photo']['man_photo'][$type]['video_type'], UPLOAD_PHOTO, $file_name . $i)) {
                                    $photoUpdate['link_video'] = $video;
                                    $d->where('id', $id_insert);
                                    $d->update('photo', $photoUpdate);
                                    unset($photoUpdate);
                        ...
                        $func->transfer(luudulieubiloi, "photo?act=man_photo&type=" . $type . "&p=" . $curPage, false);
                    }
                }
            }
            ...
            if (!empty($response)) {
                ...
                $message = base64_encode(json_encode($response));
                $flash->set('message', $message);
                $func->redirect("photo?act=add_photo&type=" . $type);
            }
            ...
            if ($success == false) {
                ...
                if ($savehere) {
                    $func->transfer(luudulieuthanhcong, "photo?act=edit_photo&type=" . $type . "&p=" . $curPage . "&id=" . $id_insert);
                } else {
                    $func->transfer(luudulieuthanhcong, "photo?act=man_photo&type=" . $type . "&p=" . $curPage);
                }
            }
        }

        $func->transfer(dulieurong, "photo?act=man_photo&type=" . $type . "&p=" . $curPage, false);
    }
}
...
function deletePhoto()
{
    ...
    if ($id) {
        ...
        if (!empty($row)) {
            ...
            $func->transfer(xoadulieuthanhcong, "photo?act=man_photo&type=" . $type . "&p=" . $curPage);
        } else {
            $func->transfer(xoadulieubiloi, "photo?act=man_photo&type=" . $type . "&p=" . $curPage, false);
        }
