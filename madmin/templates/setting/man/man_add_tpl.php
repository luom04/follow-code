...
<section class="content">
    <form class="validation-form" novalidate method="post" action="<?= $linkSave ?>" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        <div class="custom-control custom-radio d-inline-block mr-3 text-md">
                            <input class="custom-control-input mailertype" type="radio" id="mailertype-host"
                                name="data[options][mailertype]" <?= ($options['mailertype'] == 1 || $options['mailertype'] == 0) ? "checked" : "" ?> value="1">
            ...
            <div class="card-body">
                            ...
                            <?php foreach ($config['website']['lang'] as $k => $v) { ?>
                                <div class="custom-control custom-radio d-inline-block mr-3 text-md">
                                    <input class="custom-control-input" type="radio" id="lang_default-<?= $k ?>"
                                        name="data[options][lang_default]" <?= ($k == 'vi' ? "checked" : ($k == $options['lang_default'])) ? "checked" : "" ?> value="<?= $k ?>">
                ...
                <div class="row">
                    <div class="form-group col-md-4 col-sm-6">
                        ...
                        <input type="text" class="form-control text-sm" name="data[options][mst]" id="mst"
                            placeholder="Mã số thuế"
                            value="<?= (!empty($flash->has('mst'))) ? $flash->get('mst') : @$options['mst'] ?>"
                            required>
                    </div>
                    ...
                    <?php if (isset($config['setting']['fanpage']) && $config['setting']['fanpage'] == true) { ?>
                        ...
                        <div class="form-group col-md-4 col-sm-6">
                            ...
                            <input type="text" class="form-control text-sm" name="data[options][youtube]" id="youtube"
                                placeholder="Youtube"
                                value="<?= (!empty($flash->has('youtube'))) ? $flash->get('youtube') : @$options['youtube'] ?>">
                        </div>
                    <?php } ?>
                    ...
                    <div class="form-group col-md-4 col-sm-6">
                        ...
                        <input type="text" class="form-control text-sm" name="data[options][subtitle]" id="subtitle"
                            placeholder="<?= "Tiêu đề phụ" ?>"
                            value="<?= (!empty($flash->has('subtitle'))) ? $flash->get('subtitle') : @$options['subtitle'] ?>">
                    </div>
                    */ ?>
                </div>
                    ...
                    <div class="form-group">
                        <label for="coords_iframe">
                            ...
                            <a class="text-sm font-weight-normal ml-1" href="https://www.google.com/maps" target="_blank"
                                title="<?= laymanhunggooglemap ?>">(<?= laymanhung ?>)</a>
                        </label>
                        <textarea class="form-control text-sm" name="data[options][coords_iframe]" id="coords_iframe"
                            rows="5"
                            placeholder="<?= toadogooglemapiframe ?>"><?= $func->decodeHtmlChars($flash->get('coords_iframe')) ?: $func->decodeHtmlChars(@$options['coords_iframe']) ?></textarea>
                    </div>
                <?php } ?>
                <?php if (isset($config['setting']['footer']) && $config['setting']['footer'] == true) { ?>
                    <div class="form-group">
                        <label for="footer">Footer :</label>
                        <textarea class="form-control for-seo text-sm form-control-ckeditor " name="data[options][footer]"
                            id="footer" rows="5"
                            placeholder="Footer "><?= $func->decodeHtmlChars($flash->get('footer')) ?: $func->decodeHtmlChars(@$options['footer']) ?></textarea>
                    </div>
                <?php } ?>
                <div class="form-group">
                ...
                <div class="form-group">
                    <label for="tokengsc"><span>Google token GSC (Bạn phải có quyền là chủ sở hữu):</span> <a
                            href="<?= @$options['website'] ?>/madmin/api/gsc_auth.php" target="_blank">Tạo token
                            GSC</a></label>
                    <textarea class="form-control text-sm" name="data[tokengsc]" id="tokengsc" rows="5"
                        placeholder="Google token GSC"><?= $func->decodeHtmlChars($flash->get('tokengsc')) ?: $func->decodeHtmlChars(@$item['tokengsc']) ?></textarea>
                </div>
                <div class="form-group">
                    <label for="tokenoauth"><a href="https://console.cloud.google.com/apis/credentials"
                            target="_blank">Thêm địa chỉ web vào token oauth</a></label>
                    <?php /*  https://console.cloud.google.com/iam-admin/iam */ ?>
                </div>
                <div class="form-group">
                    <label for="tokenoauth"><span>Google token oauth (Coppy file json từ Google Console):</span> <a
                            href="https://console.cloud.google.com/auth/clients" target="_blank">Tạo token
                            oauth</a></label>
                    <textarea class="form-control text-sm" name="data[tokenoauth]" id="tokenoauth" rows="5"
                        placeholder="Google token oauth"><?= $func->decodeHtmlChars($flash->get('tokenoauth')) ?: $func->decodeHtmlChars(@$item['tokenoauth']) ?></textarea>
                </div>
                <div class="form-group">
                                ...
                                <div class="tab-pane fade show <?= ($k == 'vi') ? 'active' : '' ?>" id="tabs-lang-<?= $k ?>"
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control for-seo text-sm" name="data[name<?= $k ?>]"
                                            id="name<?= $k ?>" placeholder="<?= tieude ?> (<?= $k ?>)"
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control text-sm" name="data[address<?= $k ?>]"
                                            id="address<?= $k ?>" placeholder="<?= diachi ?> (<?= $k ?>)"
        ...
        <div class="card card-primary card-outline text-sm">
            <div class="card-header">
                <h3 class="card-title"><?= "Cấu hình AI" ?></h3>
            </div>
            <div class="card-body">
                <div class="row">
                    <div class="form-group col-12">
                        <label for="prompt_apikey"><?= "Google API Key" ?>:</label>
                        <input type="text" class="form-control text-sm" name="data[options][prompt_apikey]"
                            id="prompt_apikey" placeholder="<?= "Google API Key" ?>"
                            value="<?= (!empty($flash->has('prompt_apikey'))) ? $flash->get('prompt_apikey') : @$options['prompt_apikey'] ?>">
                        <div>Lấy tại đây <a href="https://aistudio.google.com/apikey"
                                target="_blank">https://aistudio.google.com/</a></div>
                    </div>
                    <div class="form-group col-12">
                        <label for="prompt_linhvuc"><?= "Lĩnh vực kinh doanh" ?>:</label>
                        <input type="text" class="form-control text-sm" name="data[options][prompt_linhvuc]"
                            id="prompt_linhvuc" placeholder="<?= "Lĩnh vực kinh doanh" ?>"
                            value="<?= (!empty($flash->has('prompt_linhvuc'))) ? $flash->get('prompt_linhvuc') : @$options['prompt_linhvuc'] ?>">
                    </div>
                    <div class="form-group col-12">
                        <label for="prompt_sanpham"><?= "Sản phẩm" ?>:</label>
                        <input type="text" class="form-control text-sm" name="data[options][prompt_sanpham]"
                            id="prompt_sanpham" placeholder="<?= "Sản phẩm" ?>"
                            value="<?= (!empty($flash->has('prompt_sanpham'))) ? $flash->get('prompt_sanpham') : @$options['prompt_sanpham'] ?>">
                    </div>
                    <div class="form-group col-12">
                        <label for="prompt_dichvu"><?= "Dịch vụ" ?>:</label>
                        <input type="text" class="form-control text-sm" name="data[options][prompt_dichvu]"
                            id="prompt_dichvu" placeholder="<?= "Dịch vụ" ?>"
                            value="<?= (!empty($flash->has('prompt_dichvu'))) ? $flash->get('prompt_dichvu') : @$options['prompt_dichvu'] ?>">
                    </div>
                    <div class="form-group col-12">
                        <label for="prompt_colorh2"><?= "Màu sắc tiêu đề" ?>:</label>
                        <input type="color" class="form-control text-sm" name="data[options][prompt_colorh2]"
                            id="prompt_colorh2" placeholder="<?= "Màu sắc tiêu đề" ?>"
                            value="<?= (!empty($flash->has('prompt_colorh2'))) ? $flash->get('prompt_colorh2') : @$options['prompt_colorh2'] ?>">
                    </div>
                </div>
            </div>
        </div>
