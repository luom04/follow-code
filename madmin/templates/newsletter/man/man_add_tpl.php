<?php
$linkMan = "newsletter?act=man&type=" . $type;
$linkSave = "newsletter?act=save&type=" . $type;
if(@$item && $item['attributes']) {
    $attrs = json_decode($item['attributes'], true);
    $strThongtin = '';
    if ($attrs['hangxe']) {
        $strThongtin .= '<p>Hãng xe: <b>' . $attrs['hangxe'] . '</b></p>';
    }
    if ($attrs['dongxe']) {
        $strThongtin .= '<p>Dòng xe: <b>' . $attrs['dongxe'] . '</b></p>';
    }
    if ($attrs['phienban']) {
        $strThongtin .= '<p>Phiên bản: <b>' . $attrs['phienban'] . '</b></p>';
    }
    if ($attrs['namsx']) {
        $strThongtin .= '<p>Năm sản xuất: <b>' . $attrs['namsx'] . '</b></p>';
    }
}
?>
<!-- Content Header -->
...
<section class="content">
    <form class="validation-form" novalidate method="post" action="<?= $linkSave ?>" enctype="multipart/form-data">
        ...
        <div class="card card-primary card-outline text-sm">
            <div class="card-header">
                <h3 class="card-title">Thông tin <?=$config['newsletter'][$type]['title_main'] ?></h3>
            </div>
            <div class="card-body">
                ...
                <div class="form-group-category row">
                    ...
                    <div class="form-group col-md-4">
                        <?php 
                          $newsList = $d->rawQueryOne("select id, namevi, slugvi from #_news_list where id = ? limit 0,1", array($item['id_service']));
                        ?>
                        <label for="id_service">Dịch vụ:</label>
                        <input type="text" class="form-control text-sm" name="data[id_service]" id="id_service" placeholder="Dịch vụ" value="<?=$newsList['namevi'] ?>" required disabled>
