    ...
    <form class="validation-form" novalidate method="post" action="<?= $linkSave ?>" enctype="multipart/form-data">
            ...
            <div class="card card-primary card-outline text-sm">
                ...
                <div class="card-body">
                    <?php if (isset($config[$com][$type][$dfgallery][$val]['images_photo']) && $config[$com][$type][$dfgallery][$val]['images_photo'] == true) { ?>
                        ...
                        <?php if(!empty($item['photo_multiple'])){ $photoDetails = explode(',',$item['photo_multiple']) ?>
                        <input type="hidden" id="photo_multiple" name="data[photo_multiple]" value="<?= $item['photo_multiple'] ?>">
                        <div class="row photoUpload-zone">
                            <?php foreach($photoDetails as $photoDetail){ ?>
                            <div class="col-6 col-sm-4 col-md-3 col-lg-2 photoUpload-detail">
                                <a data-fancybox href="<?='../upload/' . $com . '/' . $photoDetail?>">
                                    <img class="rounded" onerror="this.src='<?=ASSET . THUMBS?>/250x250x1/assets/images/noimage.png';" src="<?=ASSET . THUMBS . "/250x250x1/upload/$com/" . $photoDetail?>" alt="Alt Photo">    
                                </a>
                                <div class="delete-photo-multiple">
                                    <a href="javascript:void(0)" style="width: 30px; height: 30px;" title="Xóa hình ảnh" data-photo="<?=$photoDetail?>"><i class="far fa-trash-alt"></i></a>
                                </div>
                            </div>
                            <?php } ?>
                        </div>
                    ...
                    <div class="form-group">
                        ...
                        if (isset($config[$com][$type][$dfgallery][$val]['check_photo'])) {
        ...
        <div class="card card-primary card-outline text-sm">
            ...
            <div class="card-body">
                <?php if (isset($config[$com][$type][$dfgallery][$val]['images_photo']) && $config[$com][$type][$dfgallery][$val]['images_photo'] == true) { ?>
                    <div class="form-group">
                        <div class="upload-file">
                            ...
                            <p class=""><strong >Tên file:</strong> <span><?= $item['photo'] ?></span></p>
                ...
                <?php if ((isset($config[$com][$type][$dfgallery][$val]['name_photo']) && $config[$com][$type][$dfgallery][$val]['name_photo'] == true) || (isset($config[$com][$type][$dfgallery][$val]['desc_photo']) && $config[$com][$type][$dfgallery][$val]['desc_photo'] == true) ) { ?>
                            ...
                            <div class="tab-content" id="custom-tabs-three-tabContent-lang">
                                <?php foreach ($config['website']['lang'] as $k => $v) { ?>
                                    <div class="tab-pane fade show <?= ($k == 'vi') ? 'active' : '' ?>" id="tabs-lang-<?= $k ?>" role="tabpanel" aria-labelledby="tabs-lang">
                                        ...
                                        <?php } ?>
                                        <?php if (isset($config[$com][$type][$dfgallery][$val]['desc_photo']) && $config[$com][$type][$dfgallery][$val]['desc_photo'] == true) { ?>
                                            <div class="form-group">
                                                <label for="desc<?= $k ?><?= $i ?>"><?=mota?> (<?= $k ?>):</label>
                                                <textarea class="form-control for-seo text-sm <?= (isset($config[$com][$type][$dfgallery][$val]['desc_cke_photo']) && $config[$com][$type][$dfgallery][$val]['desc_cke_photo'] == true) ? 'form-control-ckeditor' : '' ?>" name="data[desc<?= $k ?>]" id="desc<?= $k ?>" rows="5" placeholder="<?=mota?> (<?= $k ?>)"><?= @$item['desc' . $k] ?></textarea>
                                            </div>
                                        <?php } ?>
                                    </div>
                                <?php } ?>
